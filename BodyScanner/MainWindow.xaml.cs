    ...
    public partial class MainWindow : Window, INotifyPropertyChanged, IDisposable
    {
        ...
        private void startButton_Click(object sender, RoutedEventArgs e)
        {
            if (!started)
            {
                ...
                VoxelsXSlider.Maximum = 640;
                VoxelsYSlider.Maximum = 640;
                VoxelsZSlider.Maximum = 640;
        ...
        StaticKinectWindow StaticKinectWindow = new StaticKinectWindow();
        public bool mainWindowVisibility = true;
        public bool once = true;
       
        private void viewType_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            StaticKinectWindow.Top = this.Top;
            StaticKinectWindow.Left = this.Left;

            if (viewType.SelectedIndex == 0)
            {
                viewType.SelectedIndex = 1;
                this.Visibility = Visibility.Hidden;
                StaticKinectWindow.Visibility = Visibility.Visible;

                if (once)
                {
                    StaticKinectWindow.IsVisibleChanged += (senderr, args) => {
                        if (mainWindowVisibility)
                        {
                            mainWindowVisibility = false;
                            this.Visibility = Visibility.Hidden;
                        }
                        else if (!mainWindowVisibility)
                        {
